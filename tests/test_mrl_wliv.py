...
class TestWholeLiverMatchLists(unittest.TestCase):
    """Test whether current allocation is done correctly for whole liver grafts
    """

    def test_mrl(self, n_to_assess: int = 1000):
        """ Test whether match codes are correctly predicted
        for whole liver offers
        """
        ...
        # Remove patients with action code already accepted / confirmed
        df_ml = df_ml.loc[df_ml[cn.ACTION_CODE] != 'FS', :]

        print('Checking only WLiv offers')
        ...
        for sel_id, df_sel in df_ml.groupby(cn.ID_MTR):
            ...
            ml = MatchListCurrentELAS(
                ...
                type_offer=pd.unique(df_sel[cn.TYPE_OFFER]),
                obl=obligations,
            ...
            for check_col in [cn.MTCH_TIER, cn.MTCH_LAYER]:
                ...
                if not df_sel[check_col].equals(
                    ...
                    if (df_mrl.shape[0] != df_real.shape[0]):
                        raise Exception(
                            'Blood type incompatible candidates '
                            'appear on the matchlist'
                        )
                    else:
                        ...
                        if mismatches.any():
                            ...
                            print(df_mrl.loc[
                                mismatches, cg.TEST_MRL_COLS +
                                [cn.BG_RULE_COL, cn.ANY_OBL] +
                                ['real_'+check_col]].T
                                )

                            if check_col == cn.MTCH_TIER:
                                # Do not throw errors for Belgian
                                # non-residents (match tier A)
                                assert (
                                    df_real.loc[mismatches, cn.MTCH_TIER] ==
                                    cn.TIER_A
                                    ).all(), \
                                    'Error in calculation MRL.' \
                                    'Inspect the output.'
                            elif check_col == cn.MTCH_LAYER:
                                # Do not throw errors if rounded match MELD is
                                # 30 (BG-threshold appears based on unrounded
                                # match meld)
                                assert (
                                    (df_real.loc[
                                        mismatches,
                                    ...
                                    f'Error in calc. MRL. Inspect the output.'

    def _return_deepest_values(self, in_dict: Dict):
        """Return deepest values for nested dictionary."""
        if isinstance(in_dict, Dict):
            if not any([isinstance(v, dict) for v in in_dict.values()]):
                return tuple(list(in_dict.values()))
            else:
                return sum([
                    self._return_deepest_values(nd) for nd in in_dict.values()
                    ],
                    ()
                )
        else:
    ...
    def _check_allocation_rules(self, rule_dict: Dict[str, Any]) -> None:
        """Function whether tuples in allocation rules have correct length."""
        for rules in self._return_deepest_values(rule_dict):
            for rule in rules:
                for cond_tuple in rule[1]:
                    assert len(cond_tuple) == 2, \
                        f'Following tuple is wrong length: {cond_tuple}'

    def test_rec_elig_tabs(self) -> None:
        """Test recipient eligibility tables"""
        self._check_allocation_rules(RECIPIENT_ELIGIBILITY_TABLES)

    def test_bg_compatibility_tab(self) -> None:
        """Test recipient eligibility tables"""
        self._check_allocation_rules(BG_COMPATIBILITY_TAB)

    def test_bg_priority_rules(self) -> None:
        """Test recipient eligibility tables"""
        self._check_allocation_rules(BG_PRIORITY_RULES)

    def test_main_rank_tiers(self) -> None:
        """Test recipient eligibility tables"""
        self._check_allocation_rules(MAIN_RANK_TIERS)

    def test_first_layer_tiers(self) -> None:
        """Test recipient eligibility tables"""
        self._check_allocation_rules(FIRST_LAYER_TIERS)
