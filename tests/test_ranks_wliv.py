...
class TestWholeLiverMatchListRanks(unittest.TestCase):
    """ Test whether whole-liver match lists yield the exact
        same order as match lists exported from the ET
        data warehouse
    """

    def test_mrl(self, n_to_assess: int = 500):
        """Test whether whole-liver match list ranks are exactly reproduced.
        """
        ...
        ss = read_sim_settings(
            os.path.join(
                es.DIR_SIM_SETTINGS, 'sim_settings_tests.yml'
            )
        )
        ...
        for sel_id, df_sel in groupby(
            ...
            # Initialize synthetic obligations to real obligations at
            # moment of ML.
            seg = SEG(sim_set=ss)
            ...
            # as the order for international center offers is wrong
            # for many match lists.
            if (
