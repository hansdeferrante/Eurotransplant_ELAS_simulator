...
from simulator.code.entities import ExceptionSystem
...
class PostTransplantPredictor:
    ...
    Methods
    -------
    calculate_survival(offer, time, split, verbosity):
        calculate survival probability at time
    calculate_survival_time(offer, surv, split, verbosity):
        calculate survival time
    simulate_failure_date(offer, surv, split, verbosity):
    ...
    def __init__(
        ...
        seed: int,
        se_sys: Optional[ExceptionSystem] = None,
    ...
    def _calculate_weibull_param(
            ...
            split: int = 0, verbose=False
    ) -> float:
        """
            Calculate LP for Weibull model (both scale & shape)
        """

        # Add whether it is split or not to the offer data.
        offer.__dict__[cn.GRAFT_SPLIT] = split
        if split and offer.__dict__[cn.DELTA_WEIGHT_NOSPLIT_MA30] != 0:
            offer.__dict__[cn.DELTA_WEIGHT_NOSPLIT_MA30] = 0
    ...
    def generate_synthetic_reregistration(
        ...
        offer.__dict__[cn.HU_ELIGIBLE_REREG] = (
            offer.__dict__[cn.TIME_TO_REREG] <
            es.CUTOFF_RETX_HU_ELIGIBLE
        )
        ...
        if synth_patient.future_statuses:
            if (
                ...
                event_types_to_remove = [cn.SE, cn.NSE, cn.PRF]
            else:
                event_types_to_remove = [cn.PRF]
            synth_patient.future_statuses.remove_status_types(
                remove_event_types=event_types_to_remove
            )

        assert self.se_sys, \
            'Need to initialize PostTransplantPredictor with ' \
            'an exception system to generate synthetic updates'
